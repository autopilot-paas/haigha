...
class Connection(object):
    ...
    def connect(self, host, port):
        ...
        # state might have been set to True due to a socket error or a
        # redirect.
    ...
    def read_frames(self):
        ...
        try:
            ...
            self.close(reply_code=501, reply_text='frame error from %s : %s' % (
                self._host, str(e)), class_id=0, method_id=0, disconnect=True)
            raise ConnectionClosed("connection is closed: %s : %s" %
                                   (self._close_info['reply_code'], self._close_info['reply_text']))
    ...
    def send_frame(self, frame):
        ...
        if len(buf) > self._frame_max:
            self.close(reply_code=501, reply_text='attempted to send frame of %d bytes, frame max %d' % (
                len(buf), self._frame_max), class_id=0, method_id=0, disconnect=True)
            raise ConnectionClosed("connection is closed: %s : %s" %
                                   (self._close_info['reply_code'], self._close_info['reply_text']))
